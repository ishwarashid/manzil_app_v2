...
class RideRequestsScreenState extends ConsumerState<RideRequestsScreen> {
  List<Map<String, Object>> getRidesData() {
    final List<Map<String, String>> ridesData = [
      {
        "id": "BGUSqNr6DjoxaMuLSWWv",
        "passengerName": "Azan Rashid",
        "pickupLocation": "Tariq Bin Ziad Colony",
        "destination": "COMSATS University, Sahiwal"
      },
      {
        "id": "pDoHEwxBavtAxxjLmz9X",
        "passengerName": "Ishwa Rashid",
        "pickupLocation": "Tariq Bin Ziad Colony",
        "destination": "COMSATS University, Sahiwal"
      },
    ];
  ...
  void _initiateChat(Map<String, Object> request) async {
    final currentUser = ref.read(currentUserProvider);  // Access current user from provider
    final receiverId = request["id"];  // Assuming "passengerName" is unique
    final chatService = ChatService();

    await chatService.createChatRoom(currentUser, receiverId as String);

    // Navigate to the chat screen after the chat room is created/checked
    Navigator.push(
      context,
      MaterialPageRoute(
