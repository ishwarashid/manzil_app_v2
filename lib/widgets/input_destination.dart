...
import 'package:manzil_app_v2/services/location/location_service.dart';
...
class _InputDestinationState extends ConsumerState<InputDestination> {
  ...
  void _onSearchChanged() {
    // Reset coordinates when text changes
    setState(() {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 500), () {
      if (_destinationController.text.length >= 3) {
        _getSuggestions(_destinationController.text);
      } else {
        setState(() {
          _suggestions = [];
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Stack(  // Wrapping entire content in Stack
        children: [
          Column(
            children: [
              const Text(
          ...
          if (_showSuggestions)
            Positioned(
              top: 160, // Adjusted to appear below TextField
              left: 0,
              right: 0,
              child: Container(
                ...
                child: Material( // Added Material widget for proper elevation
